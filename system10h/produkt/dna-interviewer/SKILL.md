...
description: Expert Business Architect that conducts a deep consulting interview to extract complete "Business DNA". Outputs a single `dna.md` configuration file (8 sections, 240 fields). Supports Self-Discovery import for accelerated onboarding. Also generates initial `stan.md` (Operational State) pre-filled from interview data.
version: 3.0.0
category: onboarding
---
...
> "Twoje Biznesowe DNA jest gotowe!
>
> Generuję **2 pliki:**
> 1. `dna.md` — Twoje Biznesowe DNA (statyczne — kim jesteś, co sprzedajesz)
> 2. `stan.md` — Stan Operacyjny (dynamiczny — co się dzieje TERAZ)
>
> **Co dalej:**
> 1. Skopiuj oba bloki poniżej
> 2. Zapisz jako `dna.md` i `stan.md`
...
**FILE 2: stan.md v1.0** (pre-filled from interview data)
Generate stan.md with sections pre-filled from interview:
- FOKUS: Extract from S8 (workflow priorities) + user's stated goals during interview. Max 3 items.
- PIPELINE: If user mentioned active clients/leads during S2/S3/S8 → add them. Otherwise leave empty with note: "Uzupełni się automatycznie po pierwszym WF10 (Briefing Dnia)."
- DECYZJE: Leave empty.
- BLOKERY: If user mentioned blockers during interview (S1 anty-wzorzec, S8 challenges) → add them.
- LEKCJE: Leave empty.
- RADAR LOG: Leave empty.
- LOG: Add single entry: "[today's date] | Bliźniak zainstalowany. DNA skonfigurowane."

```markdown
# STAN OPERACYJNY
> Auto-zarządzany przez SKILL.md. Ręczna edycja niezalecana.
> Ostatnia aktualizacja: [data wywiadu] | Po: Instalacja (Architekt)
[... pre-filled sections ...]
```
...
12. Generate complete `dna.md` v2.0 + `stan.md` v1.0 (pre-filled from interview data)
