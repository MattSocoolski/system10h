# OFERTA: ARTI B2B - ADRE ARTO & ARTNAPI
**(Wersja dla Resellerów i Eventów - Edycja 2026)**

> TERMINOLOGIA: Ceny = ceny B2B netto ALL-IN (z dostawą w cenie). Marże i prowizja → wbudowane w cennik (model paletowy). Źródło: Model_paletowy_rabaty.xlsx (12.02.2026).

## 📌 TAGLINE
Bezpieczny import bez ryzyka. Towar dostępny w 24h z polskiego magazynu. Jedna cena — dostawa wliczona.

## 👤 DLA KOGO
Sklepy i Sprzedawcy, którzy chcą importowych cen, ale polskiej obsługi, rękojmi i dostępności "na wczoraj".

## 🔥 PROBLEMY KTÓRE ROZWIĄZUJEMY
1.  **Loteria Jakościowa (Podobrazia):** Koniec z "bananami" i wilgotnym drewnem z przypadkowych kontenerów.
2.  **Brak Nowości (Hobby):** Dostarczamy gotowe zestawy PBN, które rotują cały rok, a nie tylko w sezonie.

---

## 💰 ZASADY CENNIKA (od 12.02.2026)

**Model:** Ceny netto ALL-IN (produkt + magazyn + wydanie + dostawa). Zero dodatkowych kosztów.
**Progi:** Im więcej zamawiasz, tym taniej. Progi = pełne palety (brak pośrednich progów).
**Zamówienia "pomiędzy" progami:** Pełne palety × cena paletowa + nadwyżka × cena kartonowa.
**Źródło kalkulacji:** Model_paletowy_rabaty.xlsx — koszty: produkt + paleta (155 PLN) + magazyn + wydanie.
**Prowizja handlowca:** Wbudowana w cenę (jawny podział marży firma/handlowiec). Rośnie z wolumenem.

---

## 📦 CENNIK PALETOWY B2B (aktualny 16.03.2026)
> Źródło: PALETOWE LICZENIE CENNIK.xlsx. Ceny netto PLN, all-in z dostawą.

### PODOBRAZIA (płótna malarskie)

| Rozmiar | Min szt | Kartony | 1 paleta | 2 palety | 3 palety |
|---------|---------|---------|----------|----------|----------|
| **40x50** | 120 | 10,30 (120-319) | 9,00 (320) | 8,15 (640) | 7,30 (960) |
| **30x40** | 120 | 7,46 (120-479) | 6,75 (480) | 6,11 (960) | 5,00 (1440) |
| **20x30** | 120 | 6,70 (120-359) | 5,49 (720) | 4,54 (1440) | 3,68 (2160) |
| **20x20** | 120 | 5,96 (120-359) | 4,75 (1200) | 3,80 (2400) | 2,94 (3600) |

### PBN (Paint By Numbers)

| Produkt | Min szt | Kartony | 1 paleta | 2 palety | 3 palety |
|---------|---------|---------|----------|----------|----------|
| **PBN rama 40x50** | 30 | 27,64 (30-180) | 22,37 (180) | 20,40 (360) | 17,37 (720) |
| **PBN bez ramy** | 75 | 20,24 (75-449) | 16,25 (450) | 14,75 (900) | 11,72 (1350) |

### DESKI (PBN na desce)

| Produkt | Min | Kartony | 1 paleta | 2 palety | 3 palety |
|---------|-----|---------|----------|----------|----------|
| **30x40 deska (zestaw 8szt)** | 18 zest | 24,27 (18-299) | 23,33 (300) | 20,41 (600) | 19,27 (1200) |
| **40x50 deska (3szt)** | 18 | 7,88 (18-335) | 6,84 (336) | 6,24 (960) | 5,10 (1440) |
| **24x30 deska** | 45 | 16,84 (45-720) | 15,20 (720) | 14,58 (960) | 13,44 (1440) |

### SZTALUGI

| Produkt (kod) | Min | Kartony | 1 paleta | 2 palety | 3 palety |
|---------------|-----|---------|----------|----------|----------|
| **Stołowa 52-64cm (3354)** | 40 | 36,49 (40-140) | 26,49 (156) | 22,63 (312) | 21,49 (468) |
| **Mała L 46cm (3357)** | 48 | 15,32 (48-384) | 10,32 (432) | 8,82 (864) | 7,32 (1296) |
| **Mała M 36cm (3498)** | 40 | 9,48 (40-560) | 7,98 (576) | 7,12 (1152) | 5,98 (1728) |
| **Stołowa 42cm (3355)** | 40 | 16,12 (40-140) | 14,62 (252) | 13,76 (504) | 12,62 (756) |

### INNE

| Produkt | Min | Kartony | 1 paleta | 2 palety | 3 palety |
|---------|-----|---------|----------|----------|----------|
| **Haft diamentowy** | 100 | 14,08 (100-1199) | 12,58 (1200) | 11,08 (2400) | 9,58 (3600) |
| **Ramka A4** | 60 | 21,94 (60-319) | 19,44 (320) | 18,44 (640) | 17,44 (960) |

---

## 📦 KATEGORIA 1: PODOBRAZIA (Marka: Adré Artó)

### CO KLIENT DOSTAJE?
* **Produkt:** Podobrazia bawełniane Adré Artó (Marka Własna).
* **Specyfikacja:** Drewno iglaste (sosnowe), krosno standard, gramatura ~280g/m².
* **Sztalugi:** Import "economy" (szary karton) - idealny tani upsell.
* **Logistyka:** Magazyn Sokołów (Polska). Wysyłka 24h. Dostawa WLICZONA w cenę.

**Zasada dla Klienta:**
"Chcesz najlepszą cenę? Zamów pełne palety. Przy zamówieniu 500 szt: 320 × 9,00 + 180 × 10,30. Lepiej dobierz do 640 — cena spada do 8,15 all-in."

---

## 🎨 KATEGORIA 2: ZESTAWY PBN — MALOWANIE PO NUMERACH (Marka: ARTNAPI)

### CO KLIENT DOSTAJE?
* **Produkt:** Zestaw PBN na desce (Retail Ready).
* **W Pudełku:** Płótno/deska, świeże farby akrylowe (próżniowe pakowanie), 3 pędzle, haczyki.
* **Wzory:** Mix bestsellerów (470 wzorów w rotacji).

* *Atut:* Marża dla sklepu >100% (Sugerowana cena detaliczna 49-69 zł).
* Eleganckie opakowanie, 470 wzorów do wyboru, farby świeże (próżniowe pakowanie)
* Flagowy produkt Artnapi — najwyższa marża w portfolio

---

## 💰 ZASADY GRY (Polityka cenowa i negocjacji)

**Ceny:** Twarda tabela progowa — ceny jak w cennikach powyżej. Nie podlegają negocjacji.
**Rabaty:** Standardowo BRAK. Tabela cenowa jest ostateczna.
- Wyjątek: przy ~1500 szt jest cień pola do rozmowy (bardzo mało prawdopodobne)
- PBN dla retailu: tu jest elastyczność cenowa
- Zasada: nie walczymy ceną, walczymy dostępnością i pewnością

**Płatności:** Wyłącznie PRZEDPŁATA. Brak kredytów kupieckich, brak faktur z odroczonym terminem. Powód: polityka firmy (ochrona przed ryzykiem).

**Gwarancja/Zwroty:** Zwroty bez problemu, wymagana dokumentacja. Bezbolesne dla klienta — filozofia "klient ma mieć spokój".

---

## 🏷️ USP — "NIE NAJTAŃSI, ALE PEWNI"

Event Grade 280g, potrójnie gruntowane. Duży magazyn pod Warszawą, towar zawsze dostępny. Klient kupuje SPOKÓJ, nie płótno.

### Komunikat #1 (obowiązkowy w każdym outreachu)
"[X] sztuk na stanie, wysyłka 24h z polskiego magazynu."
→ Aktualizuj [X] co tydzień (piątek, po sprawdzeniu stanów).
→ Stan na 23.02: 10 000 szt 40×50 (kontener dojechał) + zapas w magazynie.

### Dowody społeczne
- 4000 płócien sprzedawanych miesięcznie
- 10 000 sztuk podobrazi 40x50 na stanie
- Powracający klienci eventowi (repeat orders co 2-6 tyg.)

---

## ⚔️ BATTLE CARDS (obsługa obiekcji)

### BC1: "Za drogo"
1. Przyznaj rację: "Rozumiem i ma Pan rację"
2. Wyjaśnij logicznie: "Zmienił się sposób rozliczenia, nie marża"
3. Daj pewność: "Mogę zagwarantować stałą cenę na kwartał"
4. Superbroń — dostępność: "Mamy na stanie 10 000 sztuk, wysyłka 24h"
5. Nie naciskaj: "Jak będzie Pan gotowy, proszę dać znać"

### BC2: "Muszę to przemyśleć"
"Jasne, zero pośpiechu. Jedno tylko — mam teraz [X] sztuk na stanie, a ruch na płótnach jest spory. Mogę trzymać rezerwację przez [X dni], żebyście mieli pewność dostępności. Daj znać jak będziecie gotowi."
→ Pilność przez dostępność, nie presję.

### BC3: "Phoenix/konkurencja ma taniej"
"Phoenix rzeczywiście ma niższą cenę na papierze. Ale przy większych zamówieniach nie mają takich stanów magazynowych — a w eventach liczy się to, że towar jest na już. Nasze płótna to Event Grade 280g, potrójnie gruntowane — klient na evencie widzi różnicę. Robimy 4000 sztuk miesięcznie, więc dostępność jest zawsze. Nie najtańsi, ale pewni — i to jest warte te kilkadziesiąt groszy różnicy na sztuce."

**Wersja "Phoenix Down" (deep research 23.02):**
"Phoenix zależy od jednego łańcucha dostaw z Chin przez Hiszpanię — i regularnie mają przerwy w dostępności. My mamy magazyn pod Warszawą z 10 000 szt na stanie. Przy evencie na 30 osób w piątek liczy się to, że towar JEST — nie że jest o złotówkę taniej."

### BC4: "Mam już dostawcę / mam kontrakt"
"Rozumiem, nie namawiam do zmiany. Ale gdybyście kiedyś potrzebowali towaru na już — a obecny dostawca akurat nie miał — dajcie znać. Mamy magazyn pod Warszawą, 10 000 sztuk na stanie. Zostawiam kontakt, zero zobowiązań."
→ Pozycjonowanie jako Plan B, który staje się Planem A.

### Wzorzec obiekcyjny
Mateusz broni WARTOŚCI, nie ceny. Przyznaje rację, tłumaczy logicznie, kładzie na stół DOSTĘPNOŚĆ jako superbroń. Nie naciska — otwiera drzwi i czeka.

---

## ✅ DLACZEGO MY? (STACK WARTOŚCI)

| Cecha | Samodzielny Import (Ryzyko) | **Współpraca z NAPI (Bezpieczeństwo)** |
| :--- | :--- | :--- |
| **Dostępność** | 60-90 dni (Czekanie na statek) | **24h (Kurier z PL)** |
| **Reklamacje** | Walka z agentem w Chinach | **Wymiana w Polsce (Faktura PL)** |
| **Płynność** | Zamrażasz gotówkę na 3 m-ce | **Płacisz przy wysyłce** |
| **Jakość** | Co przyjdzie, to sprzedajesz | **Towar zweryfikowany przez markę** |

---

## 📊 BENCHMARKING KONKURENCJI — PODOBRAZIA 40x50 (stan: 23.02.2026)

| Dostawca | Cena netto | Z dostawą? | Realna cena all-in | Status |
|----------|-----------|-----------|-------------------|--------|
| ??? (dostawca Pstro) | ~6,30 | ✅ tak | ~6,30 | Nieznany gracz PL |
| **ARTNAPI 3 pal** | **7,31** | **✅ tak** | **7,31** | ✅ |
| **ARTNAPI 2 pal** | **8,20** | **✅ tak** | **8,20** | ✅ |
| Phoenix 1000 szt | 8,85 | ❌ + transport | ~9,40-9,95 | ⚠️ Problemy z dostępnością |
| Phoenix flat | 9,00 | ❌ + transport | ~9,55-10,10 | ⚠️ Braki towaru (CNY luka) |
| **ARTNAPI 1 pal** | **9,00** | **✅ tak** | **9,00** | ✅ |
| **ARTNAPI kartony** | **10,30** | **✅ tak** | **10,30** | ✅ |
| Artmaker | 11,00 | ? | ~11,50+ | Aktywny |
| Koh-i-noor | 11,82 | ✅ od 100 szt | 11,82 | Ograniczone stany |

**Pozycjonowanie (deep research 23.02):**
- Od palety (320 szt) w górę = **ARTNAPI TAŃSZY niż Phoenix** po doliczeniu transportu
- Pod paletą = Phoenix na papierze tańszy, ale: braki towaru + transport = realna cena wyższa
- Argument: "U nas 9,00 to FINALNA cena z dostawą pod drzwi. U Phoenixa 9,00 + transport palety 200-400 PLN."

**Intel Phoenix (deep research 23.02):**
- phoenix-arts.pl, Piotr Pakuła, JDG Dąbrowa Górnicza, 2-3 osoby, 5k kapitału
- Wyłączny dystrybutor Phoenix Art Materials (Chiny) przez PAE (Hiszpania)
- Studio 280g/17×23mm (najcieńsze na rynku PL) | Artists 380g/17×38mm
- Słabość: 1 łańcuch dostaw, zero bufora, regularnie "między kontenerami"

### TAKTYKA PALETOWA (obowiązkowa od 23.02)

```
Gdy klient pyta o kartony (120-319 szt @ 10,30):
→ "Przy 320 szt cena spada do 9,00 all-in z dostawą.
   Różnica 1,30/szt × 320 = 416 PLN oszczędności.
   Dobierzecie do palety?"

Gdy klient porównuje z Phoenix:
→ "Phoenix 9,00 to bez dostawy. Transport palety = 200-400 PLN.
   U nas 9,00 to cena finalna — zero ukrytych kosztów."
```

---

## 🌍 CENNIK CEE / EXPORT (EUR netto, ALL-IN z dostawą)

**Kalkulator online:** https://artnapi.pl/B2B-Price-Calculator-cabout-pol-31.html
**Cennik HTML:** materialy-artnapi/cennik_artnapi_2026_EUR.html (per-country, do wysłania klientom)
**Kurs:** 1 EUR = 4.30 PLN
**Model:** Ceny ALL-IN (produkt + magazyn + wydanie + dostawa do miasta klienta). Zero dodatkowych kosztów.
**Formuła:** COGS = baseCost + delivery + storage + picking → FLOOR = COGS × 1.25 → tier markup
**Delivery:** Palety = palletCost / palletQty | KARTONY 40x50/30x40 = boxCost / 40 (realne koszty paczek od 10.03.2026)

### PODOBRAZIA 40x50 — per country (EUR/szt, ALL-IN z dostawą)

| Kraj | Kartony | 1 Paleta | 2 Palety | 3+ Palety |
|------|---------|----------|----------|-----------|
| 🇨🇿 CZ (Prague) | **€2.33** | **€2.03** | **€1.87** | **€1.62** |
| 🇸🇰 SK (Bratislava) | **€2.36** | **€2.25** | **€2.07** | **€1.80** |
| 🇭🇺 HU (Budapest) | **€2.36** | **€2.16** | **€1.99** | **€1.73** |
| 🇱🇹 LT (Vilnius) | **€2.31** | **€2.04** | **€1.88** | **€1.63** |
| 🇱🇻 LV (Riga) | **€2.32** | **€2.11** | **€1.95** | **€1.69** |
| 🇪🇪 EE (Tallinn) | **€2.35** | **€2.21** | **€2.03** | **€1.76** |
| 🇷🇴 RO (Bucharest) | **€2.70** | **€2.41** | **€2.21** | **€1.92** |

### PODOBRAZIA 30x40 — per country (EUR/szt, ALL-IN z dostawą)

| Kraj | Kartony | 1 Paleta | 2 Palety | 3+ Palety |
|------|---------|----------|----------|-----------|
| 🇨🇿 CZ (Prague) | **€1.86** | **€1.60** | **€1.45** | **€1.18** |
| 🇸🇰 SK (Bratislava) | **€1.89** | **€1.77** | **€1.58** | **€1.31** |
| 🇭🇺 HU (Budapest) | **€1.89** | **€1.70** | **€1.53** | **€1.26** |
| 🇱🇹 LT (Vilnius) | **€1.84** | **€1.61** | **€1.45** | **€1.19** |
| 🇱🇻 LV (Riga) | **€1.85** | **€1.67** | **€1.50** | **€1.23** |
| 🇪🇪 EE (Tallinn) | **€1.88** | **€1.74** | **€1.55** | **€1.29** |
| 🇷🇴 RO (Bucharest) | **€2.23** | **€1.90** | **€1.67** | **€1.41** |

### KOSZTY PALET per kraj (PLN, źródło: 1000pcs offer)

| Kraj | Koszt palety (PLN) |
|------|--------------------|
| CZ | 430 |
| SK | 649.3 |
| HU | 563.3 |
| LT | 442.9 |
| LV | 516 |
| EE | 606.3 |
| RO | 804.1 |
| PL (domestic) | 155 |

### KOSZTY PACZEK per kraj (PLN/box, 40 szt per box)

| Kraj | Koszt paczki (PLN) |
|------|--------------------|
| CZ | 49.88 |
| SK | 53.63 |
| HU | 54.00 |
| LT | 46.78 |
| LV | 47.84 |
| EE | 51.90 |
| RO | 100.15 |

**Uwaga:** Ceny KARTONY od 10.03.2026 kalkulowane z realnym kosztem paczki (nie prorata z palety). Dotyczy tylko 40x50 i 30x40.

**Zasada CSO:** ZAWSZE kieruj na kalkulator online (priorytet) LUB wyślij cennik HTML jako załącznik (fallback).
**URL kalkulatora:** https://artnapi.pl/B2B-Price-Calculator-cabout-pol-31.html
**Wzorzec PL:** "Przygotowałem interaktywny kalkulator cen — wybierz produkt i ilość, zobaczysz cenę all-in z dostawą: [URL]"
**Wzorzec EN (CEE):** "I've prepared an interactive price calculator — select your country to see all-in pricing incl. delivery: [URL]"
**PKE Mail 2 (po zgodzie):** "Przygotowałem dla Was interaktywny kalkulator cen: [URL]. Wybierz produkt, ilość — zobaczysz cenę all-in z dostawą. Chętnie omówię szczegóły telefonicznie."
**Pełna lista produktów w EUR:** 9 produktów (4 podobrazia, 2 panele, 3 PBN/hobby) — patrz cennik HTML lub kalkulator.
**GHOST GATE:** Każda wiadomość wychodząca z linkiem do kalkulatora MUSI przejść przez @ghost (ghost_styl.md B2B/EN).

---

## INSTRUKCJE DLA AI (Asystenta Sprzedaży)
* **O Podobraziach (Adré Artó):** "To nasz bezpieczny import. Bierzemy ryzyko jakości na siebie. Ty dostajesz sprawdzony towar z polską fakturą."
* **O PBN (Artnapi):** "To gotowiec na półkę. 21 zł netto. Kupujesz karton, sprzedajesz z marżą 100%. Żadnych zwrotów, bo farby są świeże."